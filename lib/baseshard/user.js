      ...
      user.auth(function (err) {
        if (typeof err !== 'undefined') {
          if (err.err === 'AUTH_FAILED') {
            ...
            log.store();
            return res.send({err: 'INVALID_OLD_PASSWORD', msg: 'Invalid old password.'});
          } else {
            console.log(util.inspect(err));
            return res.status(500).send();
          }
        ...
        log.store();
        req.session.user = user;
        db.update({name: req.session.user.name}, {$set: {password: user.hash(req.body.password)}}, 'users', {}, function (err) {
          if (err) {
            console.log(util.inspect(err));
            return res.status(500).send();
          }
    ...
    respond: function (req, res, db) {
      ...
      db.find({name: req.session.user.name}, 'users', {limit: 1}, function (err, docs) {
        ...
        if (docs[0].email != req.query.email) {
          var log = new Log(req, 'Account removal attempt: ' + req.session.user.name + ' ' + req.query.email, 'ACCOUNT_DELETE_FAIL');
          log.store();
          return res.send({err: 'EMAIL_ADDR_MISMATCH', msg: 'This email address is not registered to this account.'});
        }
