...
module.exports = [
  ...
  },
  {
    ...
    respond: function (req, res, db) {
      ...
      if (typeof req.files === 'undefined' || typeof req.files.img === 'undefined' || typeof req.params.shard === 'undefined') {
        return res.status(404).render('404.hbs', {path: req.originalUrl});
      }
      var imgStream = fs.createReadStream(req.files.img.path);
      var writeName = req.files.img.name.replace(/\.[a-zA-Z]+$/g, '') + '_' + new Date().getTime() + '.jpg';
      var writePath = process.cwd() + '/usercontent/usercontent/' + req.params.shard;
      gm(imgStream, req.files.img.name).quality(80).write(writePath + writeName, function (err) {
        if (err) {
          console.log('Image upload error: ' + writePath + '\nErr: ' + util.inspect(err));
